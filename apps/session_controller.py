...
def check_credentials(papp_to_run=''):
    ##function assums g.SEC has been initialized.
    if papp_to_run == '':
        _app = g.APPSTACK.get(g.ENVIRO.get('PYAPP_TO_RUN', ''), {})
    else:
        _app = g.APPSTACK.get(papp_to_run, {})

    if _app.get('security', True): ## default to True if not defined it blocks access by default
        _ua = g.SEC.get('USER_ACCESS')
...
def load_session(p_session_id = None):
    ...
    if p_session_id is None or not hasattr(p_session_id, 'value') :
        ...
        elif _session.get('TIMEOUT')< _ctime and g.SEC['USER_AUTOLOGIN']:
            ## session has timeout but the autologin is turned on so log the user in and continue
            if load_credentials(g.COOKIES.get('user', ''), g.COOKIES.get('pwd', ''), p_con ):
...
def create_session():
    ...
    g.CLIENT_STATE.update({'TIMEOUT': dt.utcnow() + td(seconds=g.SEC.get('USER_TIMER', 600))})
    save_session(g.CLIENT_STATE, session_id, '')

def save_session(pdict=None, p_session_id=None, p_last_command='' ):
    ...
    q_sql = """ insert into client_state values (
                ...
                set cs_data = %(pdict)s
             """
    _rec = m.run_sql_command(q_sql, {'p_session_id': p_session_id,
...
def set_cookie(pname='', pvalue='', pexpires=None, pdomain=None,
    ...
    g.COOKES_to_Send.update({name:morsel})
